...
このrepositoryの [Evidence Budget](確認する根拠の量を決める.md) は、Required evidenceと探索停止条件を管理する手法です。context costの概算とは役割が異なります。
...
[Context Manifest](参照先の目録.md) は候補pointerのboundedな目録です。
