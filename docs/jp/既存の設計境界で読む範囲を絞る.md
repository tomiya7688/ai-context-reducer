...
[Responsibility Map](ファイルとモジュールの役割表.md) は「どのfile / moduleが何を担当するか」を短く示します。
...
[Change Routing Map](変更内容ごとの案内表.md) は変更カテゴリからsource / tests / docsへ直接routingします。
...
[Source Structure Index](コード構造の索引.md) はsymbol / import / dependency等の機械的な構造情報から、必要な原典へ到達するための索引です。
