...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('root',nargs='?',default='.'); ap.add_argument('--limit',type=int,default=1000); a=ap.parse_args(); root=Path(a.root).resolve(); rows=[]; exts={'.cpp','.cc','.cxx','.hpp','.hh','.hxx','.h'}
