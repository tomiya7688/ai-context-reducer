...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('root',nargs='?',default='.'); ap.add_argument('--limit',type=int,default=2000); a=ap.parse_args(); root=Path(a.root).resolve(); exts={'.cpp','.cc','.cxx','.hpp','.hh','.hxx','.h'}; files=[]
    for p in sorted(root.rglob('*')):
        if p.is_file() and p.suffix.lower() in exts: files.append(p)
