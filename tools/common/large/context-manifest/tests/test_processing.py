...
class ContextManifestTests(unittest.TestCase):
    # test_priority_routes_high_source_tests_docs_then_otherでmanifestのbounded性・priority・warning契約が回帰していないことを検証する。
    ...
    # test_manifest_is_bounded_and_reports_warningsでmanifestのbounded性・priority・warning契約が回帰していないことを検証する。
    ...
    # test_zero_limit_returns_no_rows_but_preserves_totalでmanifestのbounded性・priority・warning契約が回帰していないことを検証する。
