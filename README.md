...
このリポジトリで独自に作成した **文書・設計内容・テンプレート・スクリプト・ユーティリティ・ツール類を含むリポジトリ本体** は、[MIT License](LICENSE) で提供します。

外部プロジェクトを参考にしたことや、その手法・考え方を一般化してこのリポジトリ向けに独自実装・独自記述したことだけで、外部プロジェクトのライセンスがこのリポジトリ本体へ適用されるものではありません。

一方、外部プロジェクトからコード・文書・素材等を実際にコピー、改変、同梱、再配布している部分がある場合、その第三者由来部分については各外部プロジェクトのライセンス条件に従います。このリポジトリの MIT License が第三者素材を再ライセンスするものではありません。
